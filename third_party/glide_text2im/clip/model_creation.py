...
class CLIPModel:
    ...
    def image_embeddings(self, images: torch.Tensor, t: Optional[torch.Tensor] = None) -> torch.Tensor:
        z_i = self.image_encoder((images + 1) * 127.5, t)
        return z_i / (torch.linalg.norm(z_i, dim=-1, keepdim=True) + 1e-12)

    def cond_fn(self, prompts: List[str], grad_scale: float, refer_prompt=None) -> Callable[..., torch.Tensor]:
        with torch.no_grad():
            z_t = self.text_embeddings(prompts)
            if refer_prompt is not None:
                z_t_source = self.text_embeddings(refer_prompt)
                z_t = z_t - z_t_source
